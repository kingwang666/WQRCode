...
import com.larswerkman.holocolorpicker.listener.OnColorSelectedListener;
...
public class ColorPicker extends View {
    ...
    private boolean mColorCenterHaloEnabled;
    ...
    private boolean mColorPointerHaloEnabled;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mColorCenterHaloEnabled = a.getBoolean(R.styleable.ColorPicker_color_center_halo_enabled, true);
        ...
        mColorPointerHaloEnabled = a.getBoolean(R.styleable.ColorPicker_color_pointer_halo_enabled, true);
        mShowCenterOldColor = a.getBoolean(R.styleable.ColorPicker_color_show_older, mShowCenterOldColor);
        int colorHalo = a.getColor(R.styleable.ColorPicker_color_halo_color, Color.argb(127, 0, 0, 0));
        a.recycle();
        ...
        mCenterHaloPaint.setColor(colorHalo);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCenterClick && mColorCenterHaloEnabled) {
    ...
    /**
     * must after {@link #calculateColor(float)}
     *
     * @return
     */
    private int getTrueColor() {
        Color.colorToHSV(mColor, mHSV);
        mHSV[1] = mSaturation;
        mHSV[2] = mValue;
        return Color.HSVToColor(mOpacity, mHSV);
    }
    ...
    public void setColor(int color) {
        ...
        mColor = Color.HSVToColor(new float[]{mHSV[0], 1.0f, 1.0f});
        mPointerColor.setColor(mColor);

        // check of the instance isn't null
        if (mOpacityBar != null) {
            // set the value of the opacity
            mOpacityBar.setColor(mHSV);
            mOpacity = Color.alpha(color);
            mOpacityBar.setOpacity(mOpacity);
        }

        if (mSaturationBar != null) {
            mSaturationBar.setColor(mHSV);
            mSaturation = mHSV[1];
            mSaturationBar.setSaturation(mSaturation);
        }
        if (mValueBar != null) {
            mValueBar.setColor(mHSV);
            mValue = mHSV[2];
            mValueBar.setValue(mValue);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (onColorSelectedListener != null && mCenterNewColor != mOldSelectColor) {
                    onColorSelectedListener.onColorSelected(mCenterNewColor);
                    mOldSelectColor = mCenterNewColor;
                }
    ...
    public void addOpacityBar(OpacityBar bar) {
        ...
        mOpacityBar.setColor(getTrueHSV());
        mOpacityBar.setOpacity(mOpacity);
    }
    ...
    public void setNewCenterColor(int color) {
        if (mCenterNewColor != color) {
            mCenterNewColor = color;
            mCenterNewPaint.setColor(color);
            if (mShowCenterOldColor && mCenterOldColor == 0) {
                mCenterOldColor = color;
                mCenterOldPaint.setColor(color);
            }
